    ...
    if (_bpp == 8) {
        ...
        if (color != 0) {
            ...
            color = (color & 0xE0) << 8 | (color & 0xC0) << 5
                | (color & 0x1C) << 6 | (color & 0x1C) << 3 | blue[color & 0x03];
        }
        return color;
    }
        ...
        for (uint32_t yp = yo; yp < yo + hs; yp++) {
            ...
            for (uint32_t xp = xo; xp < xo + ws; xp++) {
                ...
                if (_iswapBytes)
                    color = color << 8 | color >> 8;
                _img8[x + ys * _iwidth] =
                    (uint8_t) ((color & 0xE000) >> 8 | (color & 0x0700) >> 6 | (color & 0x0018) >> 3);
                x++;
            }
            ys++;
        }
...
void TFT_eSprite::setWindow(int32_t x0, int32_t y0, int32_t x1, int32_t y1)
{
    ...
    if ((x0 >= _iwidth) || (x1 < 0) || (y0 >= _iheight) || (y1 < 0)) {  // Point to that extra "off screen" pixel
        _xs = 0;
        _ys = _iheight;
        _xe = 0;
        _ye = _iheight;
    ...
    if (_bpp == 16) {
        ...
        while (h--) {           // move pixels one by one
            for (uint16_t xp = 0; xp < w; xp++) {
                if (dx <= 0)
                    drawPixel(tx + xp, ty, readPixel(fx + xp, fy));
                if (dx > 0)
                    drawPixel(tx - xp, ty, readPixel(fx - xp, fy));
            }
            if (dy <= 0) {
...
void TFT_eSprite::setRotation(uint8_t rotation)
{
    ...
    _rotation = rotation;
    if (rotation == 0 && _iwidth > _iheight)
        swap_coord(_iwidth, _iheight);
    if (rotation == 1 && _iwidth < _iheight)
        swap_coord(_iwidth, _iheight);
    if (rotation == 2 && _iwidth > _iheight)
        swap_coord(_iwidth, _iheight);
    if (rotation == 3 && _iwidth < _iheight)
        swap_coord(_iwidth, _iheight);
}
...
int16_t TFT_eSprite::drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font)
{
...
#ifdef LOAD_RLE
    {
        if ((font > 2) && (font < 9)) {
            ...
            flash_address =
                pgm_read_dword((const void *)(pgm_read_dword(&(fontdata[font].chartbl)) + uniCode * sizeof(void *)));
        ...
        // Font is not 2 and hence is RLE encoded
    {
        ...
        if (_bpp == 16)
            ...
            color = ((textcolor & 0xE000) >> 8 | (textcolor & 0x0700) >> 6 | (textcolor & 0x0018) >> 3);
...
void TFT_eSprite::drawGlyph(uint16_t code)
{
    ...
    if (found) {
                    ...
                    if (pixel != 0xFF) {
                        ...
                        if (_bpp != 1)
                            drawPixel(x + this->cursor_x + this->gdX[gNum],
                                      y + this->cursor_y + this->gFont.maxAscent - this->gdY[gNum], alphaBlend(pixel,
                                                                                                               fg, bg));
                        else if (pixel > 127)
                            drawPixel(x + this->cursor_x + this->gdX[gNum],
                                      y + this->cursor_y + this->gFont.maxAscent - this->gdY[gNum], fg);
                    } else {
                        if (dl == 0)
                            xs = x + this->cursor_x + this->gdX[gNum];
                        dl++;
                    }
                } else {
                    if (dl) {
        ...
        drawRect(this->cursor_x, this->cursor_y + this->gFont.maxAscent - this->gFont.ascent, this->gFont.spaceWidth,
                 this->gFont.ascent, fg);
...
void TFT_eSprite::printToSprite(char *cbuffer, uint16_t len)    //String string)
{
    ...
    if (newSprite) {
        ...
        if (this->textbgcolor != TFT_BLACK)
            fillSprite(this->textbgcolor);
    }
