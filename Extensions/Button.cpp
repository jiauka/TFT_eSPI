...
void TFT_eSPI_Button::initButtonG(
 TFT_eSPI *gfx, int16_t x, int16_t y, uint16_t w, uint16_t h,
...
// Newer function instead accepts upper-left corner & size
void TFT_eSPI_Button::initButtonGUL(
 TFT_eSPI *gfx, int16_t x1, int16_t y1, uint16_t w, uint16_t h,
 void *imgOn, void *imgOff,uint16_t size)
{
  _x1           = x1;
...
boolean TFT_eSPI_Button::isPressed()    { return _currstate; }
boolean TFT_eSPI_Button::justPressed()  { return (_currstate && !_laststate); }
boolean TFT_eSPI_Button::justReleased() { return (!_currstate && _laststate); }
